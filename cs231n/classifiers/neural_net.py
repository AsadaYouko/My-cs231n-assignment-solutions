...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    X1 = np.maximum(0, np.matmul(X, W1) + b1)
    ...
    loss = np.sum(- correct_class_score) + np.sum(np.log(scores_sum))
    loss /= N
    ...
    dscore = np.exp(scores) / scores_sum[..., np.newaxis]
    ...
    dW2 = np.matmul(X1_mask.T, dscore)
    dX1 = np.matmul(dscore, W2.T)
    
    dX1_mask = dX1
    dX1_mask[X1==0] = 0
    dW1 = np.matmul(X.T, dX1_mask)
    
    grads['W2'] = dW2 + 2 * reg * W2
    grads['b2'] = np.sum(dscore, axis=0)
    grads['W1'] = dW1 + 2 * reg * W1
    grads['b1'] = np.sum(dX1_mask, axis=0)
